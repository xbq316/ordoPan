...
use FindBin qw($Bin);
$DAG_position="$Bin";
my $list = shift || die "用法: perl $0 sample_list output_dir [blast_threads] [(覆盖blast结果)1|0]\n";
my $output_dir = shift || die "用法: perl $0 sample_list output_dir [blast_threads] [(覆盖blast结果)1|0]\n";
my $blast_threads = shift || 20;  # 默认值为20
my $overWrite= shift || 0; # 默认不覆盖blast结果
my @samples;
...
while (<IN>){
	...
	if (-e "$id.prot"){
		...
		system(qq(cat $id.prot | perl -npe "s/>/>$id\_/" > $output_dir/$id.prot2));
	}
	else {
 	...
 	if (-e "$id.bed"){
		...
		system(qq(cat $id.bed | sort -k1,1 -k2,2n > $output_dir/$id.bed2));
		open IN0, "$output_dir/$id\.bed2";
		open OUT0,">$output_dir/$id.coo";
		$order=0;
 		while (<IN0>){
			chomp;
		...
		close IN0;
		close OUT0;
		system(qq(rm -f $output_dir/$id.bed2));
...
foreach $i (0..$len){
	...
	# 覆盖blast结果
	if($overWrite ne 1){
		system(qq(diamond makedb --in $output_dir/$sample1.prot2 -d $output_dir/$sample1));
	}
	else{
		# 不覆盖blast结果：只有结果文件不存在时才运行
		my $result_file = "$output_dir/$sample1.dmnd";
		unless (-e $result_file) {
			system(qq(diamond makedb --in $output_dir/$sample1.prot2 -d $output_dir/$sample1));
		}
		else {print "BLAST结果已存在，跳过: $result_file\n";}
	}
	...
	foreach $j (0..$len){
		if($i ne $j){
			...
			# 覆盖blast结果
			if($overWrite ne 1){
				system(qq(diamond blastp -d $output_dir/$sample1 -q $output_dir/$sample2.prot2 -e 1e-20 --id 40 -f 6 -o $output_dir/$sample2\_$sample1.diamond -k 3 -p $blast_threads));		
			}
			else{
				# 不覆盖blast结果：只有结果文件不存在时才运行
				my $result_file = "$output_dir/$sample2\_$sample1.diamond";
				unless (-e $result_file) {
					system(qq(diamond blastp -d $output_dir/$sample1 -q $output_dir/$sample2.prot2 -e 1e-20 --id 40 -f 6 -o $result_file -k 3 -p $blast_threads));
				}
				else {print "BLAST结果已存在，跳过: $result_file\n";}
			}
			...
			open OUTdag,">$output_dir/$diamond\.dag";
			open INdia,"$output_dir/$diamond";
			while (<INdia>){
				chomp;
				my @blast=split(/\t/,$_);
				if ($blast[0] eq $temp){
			...
			##Syntenic Block
			system(qq(perl $DAG_position/run_DAG_chainer.pl -i $output_dir/$diamond\.dag -D 10 -g 1 -A 5 ));
			%Le={};
			%Ri={};
			open IN33,"$output_dir/$diamond\.dag.aligncoords";
			open OUT33,">$output_dir/$diamond\.temp";
			while (<IN33>){
				chomp;
				$seq=$_;
				if (not /^#/){
			...
			system(qq(cat $output_dir/$diamond\.temp | cut -f1,2,3,4,5,6,7,8 | sort -k5,5 -k7,7n > $output_dir/$diamond\.sg));
			system(qq(rm -f $output_dir/$diamond\.temp));
			$syn_num =`cat $output_dir/$diamond\.sg | grep -v "#" | wc -l`;
			$sample1_num =`cat $output_dir/$sample1\.coo | wc -l`;
			$sample2_num =`cat $output_dir/$sample2\.coo | wc -l`;
			print "#que:$sample2 $sample2_num vs ref:$sample1 $sample1_num $syn_num\n";
			system(qq(rm -f $output_dir/$diamond\.dag));
			system(qq(rm -f $output_dir/$diamond\.dag.aligncoords));
		}
	}
}
...
open Ref_COO,"$output_dir/$ref_coo";
while (<Ref_COO>){
	chomp;
	@ref_gene=split(/\t/,$_);
...
foreach $n (1..($genome_number-1)){
	...
	foreach $j (0..($n-1)){
		...
		open INsyn_rev,"$output_dir/$syngene_rev";
		while (<INsyn_rev>){
			chomp;
			my ($QG_chr, $QG, $QG_start, $QG_end, $RG_chr, $RG, $RG_start, $RG_end)=split(/\t/,$_);
			$synpair{$QG}=$RG;
...
%SG_gecount;
%SG_gocount;
open SG,"$output_dir/$list.SG";
open PAN,">$output_dir/$list.SG.pan";
while (<SG>){
	chomp;
	($SG, $genes)=split(/\t/,$_);
	@gene=split(/\,/,$genes);
...
###***STAT***
open STAT,">$output_dir/$list.sg.stat";
